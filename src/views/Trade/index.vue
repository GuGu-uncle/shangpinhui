...
<script>
  ...
  export default {
    ...
    setup(){
      ...
      const msg = ref('')
      // 提交订单成功返回的id
      const orderId = computed(() => store.state.trade.orderId)
      // 点击提交触发的函数
      const submitOrder = async () => {
        // 订单参数
        let data = {
            consignee: userDefaultAddress.value.consignee || '我爱你',
            consigneeTel: userDefaultAddress.value.phoneNum || '13421298765',
            deliveryAddress: userDefaultAddress.value.fullAddress || '北京',
            paymentWay: "ONLINE",
        ...
        try {
          // 发送请求提交订单
          await store.dispatch("submitOrder",{tradeNo:orderInfo.value.tradeNo,data})
