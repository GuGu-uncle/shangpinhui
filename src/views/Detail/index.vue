        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="skuSaleAttr in skuInfo.skuSaleAttrValueList" :key="skuSaleAttr.id">
...
<script>
  ...
  export default {
    ...
    setup(){
      onMounted(() => {
        // 发送请求获取商品详情并储存在vuex中
        store.dispatch("getGoodsList",route.params.skuId)
      })
      ...
      let changeSkuNum = () => {
        // 使输入框的内容只能为正整数
        skuNum.value =handleNum(skuNum.value,1)
      }
      // 加入购物车调用的函数
      const addShopCart = async() => {
        try {
          // 发送请求
          await store.dispatch("reqAddOrUpdateShopCart",{skuId:route.params.skuId,skuNum:skuNum.value})
          // 将商品信息存储到sessionStorage
