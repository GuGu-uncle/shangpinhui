<template>
  <div>
    <TypeNav />
    <div class="main">
      <div class="py-container">
        <!--bread:面包屑（带有x的结构）-->
        <div class="bread">
          <ul class="fl sui-breadcrumb">
            <li>
              <a href="#">全部结果</a>
            </li>
          </ul>
          <ul class="fl sui-tag">
            <!-- 分类面包屑 -->
            <li class="with-x" v-show="searchParams.categoryName">
              {{searchParams.categoryName}}
              <i @click="removeCategoryName">×</i>
            </li>
            <!-- 关键字面包屑 -->
            <li class="with-x" v-show="searchParams.keyword">
              {{searchParams.keyword}}
              <i @click="removeKeyword">×</i>
            </li>
            <!-- 品牌面包屑 -->
            <li class="with-x" v-show="searchParams.trademark">
              {{searchParams.trademark && searchParams.trademark.split(":")[1]}}
              <i @click="removeTrademark">×</i>
            </li>
            <!-- 售卖属性面包屑 -->
            <li class="with-x" v-for="attrValue in searchParams.props" :key="attrValue">
              {{attrValue.split(":")[1]}}
              <i @click="removeAttr(index)">×</i>
            </li>
          </ul>
        </div>

        <!--selector-->
        <SearchSelector @trademarkInfo="trademarkInfo" @attrInfo="attrInfo"/>

        <!--排序-->
        <div class="details clearfix">
          <div class="sui-navbar">
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{active:isOne}" @click="handleOrder(1)">
                  <a>综合<span 
                    v-show="isOne" 
                    class="iconfont"
                    :class="{
                      'icon-xiangshangjiantoucuxiao':isUp,
                      'icon-xiangxiajiantoucuxiao':!isUp
                    }"
                  ></span></a>
                </li>
                <li :class="{active:!isOne}" @click="handleOrder(2)">
                  <a>价格<span 
                    v-show="!isOne" 
                    class="iconfont"
                    :class="{
                      'icon-xiangshangjiantoucuxiao':isUp,
                      'icon-xiangxiajiantoucuxiao':!isUp
                    }"
                  ></span></a>
                </li>
              </ul>
            </div>
          </div>
          <!-- 商品列表 -->
          <div class="goods-list">
            <ul class="yui3-g">
              <li class="yui3-u-1-5" v-for="goods in goodsList" :key="goods.id">
                <div class="list-wrap">
                  <div class="p-img">
                    <router-link :to="`/detail/${goods.id}`">
                      <img v-lazy="goods.defaultImg" />
                    </router-link>
                  </div>
                  <div class="price">
                    <strong>
                      <em>¥</em>
                      <i>{{ goods.price }}.00</i>
                    </strong>
                  </div>
                  <div class="attr">
                    <a
                      target="_blank"
                      href="item.html"
                      title="促销信息，下单即赠送三个月CIBN视频会员卡！【小米电视新品4A 58 火爆预约中】"
                    >
                      {{ goods.title }}
                    </a>
                  </div>
                  <div class="commit">
                    <i class="command">已有<span>2000</span>人评价</i>
                  </div>
                  <div class="operate">
                    <a
                      href="success-cart.html"
                      target="_blank"
                      class="sui-btn btn-bordered btn-danger"
                      >加入购物车</a
                    >
                    <a href="javascript:void(0);" class="sui-btn btn-bordered"
                      >收藏</a
                    >
                  </div>
                </div>
              </li>
            </ul>
          </div>
          <!-- 分页器 -->
          <Pagination 
            :pageNo="searchList.pageNo || 1" 
            :pageSize="searchList.pageSize || 1" 
            :total="searchList.total || 1" 
            :continues="5" 
            @getPageNo="getPageNo"
          />
        </div>
      </div>
    </div>
  </div>
</template>

<script>
// 引入子组件
import SearchSelector from "./SearchSelector/SearchSelector"
import { onMounted,reactive, computed,watch } from "vue"
import { useStore } from "vuex"
import {useRoute,useRouter} from "vue-router"
// 引入全局事件总线
import emitter from '@/mybus'
export default {
  name: "Search",
  components: {
    SearchSelector,
  },
  setup() {
    onMounted(() => {
      // 发送请求
      getData()
    })

    const store = useStore()
    const route= useRoute()
    const router = useRouter()
    // search搜索携带的参数
    const searchParams = reactive({
      category1Id: undefined,//一级分类ID
      category2Id: undefined,//二级分类ID
      category3Id: undefined,//三级分类ID
      categoryName: undefined,//分类名称
      keyword: undefined,//搜索关键字
      order: "1:desc",//排序方式
      pageNo: 1,//页码
      pageSize: 5,//每页数量
      props: [],//商品属性的数组: ["属性ID:属性值:属性名"]
      trademark: undefined,//品牌: "ID:品牌名称"
    })
    // 获取商品列表的数据
    const goodsList = computed(() => store.getters.goodsList)
    // 获取search搜索商品的全部数据
    const searchList = computed(() => store.state.search.searchList)
    // 判断是综合排序还是价格排序（true:综合，false:价格）
    const isOne = computed(() => searchParams.order.split(":")[0] == 1 ? true : false)
    // 判断箭头是否要向上
    const isUp = computed(() => searchParams.order.indexOf("asc") != -1 ? true : false)
    // 发送请求获取search搜索商品的数据，并将数据存储在vuex中的回调函数
    const getData = (page) => {
      // 将路由中的params、query参数添加进searchParams中
      Object.assign(searchParams,route.params)
      let {category1id,category2id,category3id,categoryname} = route.query
      searchParams.category1Id = category1id
      searchParams.category2Id = category2id
      searchParams.category3Id = category3id
      searchParams.categoryName = categoryname
      // 在没有点击页码的时候将页码置为1
      searchParams.pageNo = page || 1
      // 发送请求并存储数据
      store.dispatch("getSearchList",searchParams)
    }
    // 删除分类面包屑
    const removeCategoryName = () => {
      if(route.params.keyword){
        router.push({name:'search',params:route.params})
      }else{
        router.push({name:'search'})
      }
    }
    // 删除关键字面包屑
    const removeKeyword = () => {
      searchParams.keyword = undefined
      if(route.query.categoryname){
        router.push({name:'search',query:route.query})
      }else{
        router.push({name:'search'})
      }
      // 通过事件总线删除搜索框的keyword
      emitter.emit('clearKeyword')
    }
    // 删除品牌面包屑
    const removeTrademark = () => {
      searchParams.trademark = undefined
      // 发送请求获取数据
       getData()
    }
    // 删除售卖属性面包屑
    const removeAttr = (index) => {
      searchParams.props.splice(index,1)
      // 发送请求获取数据
      getData()
    }
    // 自定义事件，获取子组件的品牌信息`
    const trademarkInfo = (trademark) => {
      searchParams.trademark = `${trademark.tmId}:${trademark.tmName}`
      // 发送请求获取数据
      getData()
    }
    // 自定义事件，获取子组件的售卖属性信息
    const attrInfo = (attr,attrValue) => {
      let props = `${attr.attrId}:${attrValue}:${attr.attrId}`
      // 判断参数中是否存在相同的信息
      if(searchParams.props.indexOf(props) === -1){
        searchParams.props.push(props)
        // 发送请求获取数据
        getData()
      }
    }
    // 点击修改商品展示的顺序
    const handleOrder = (flag) => {
      let order = searchParams.order
      let searchFlag = order.split(":")[0]
      let searchSort = order.split(":")[1]
      let newOrder = ''
      // 判断点的是否是当前排序
      if(flag == searchFlag){
        newOrder = `${flag}:${searchSort.indexOf("asc") != -1 ? 'desc' : 'asc'}`
      }else{
        newOrder = `${flag}:desc`
      }
      searchParams.order = newOrder
      // 发送请求获取数据
      getData()
    }
    // 获取当前点击页的数据
    const getPageNo = (page) => {
      // 发送请求获取数据
      getData(page)
    }

    //监听路由,路由发生变化时，发送请求获取新的素具
    watch(route,()=>{
      getData()
    })

    return {
      searchParams,
      goodsList,
      searchList,
      isOne,
      isUp,
      removeCategoryName,
      removeKeyword,
      removeTrademark,
      removeAttr,
      trademarkInfo,
      attrInfo,
      handleOrder,
      getPageNo
    }
  },
}
</script>

<style lang="less" scoped>
.main {
  margin: 10px 0;

  .py-container {
    width: 1200px;
    margin: 0 auto;

    .bread {
      margin-bottom: 5px;
      overflow: hidden;

      .sui-breadcrumb {
        padding: 3px 15px;
        margin: 0;
        font-weight: 400;
        border-radius: 3px;
        float: left;

        li {
          display: inline-block;
          line-height: 18px;

          a {
            color: #666;
            text-decoration: none;

            &:hover {
              color: #4cb9fc;
            }
          }
        }
      }

      .sui-tag {
        margin-top: -5px;
        list-style: none;
        font-size: 0;
        line-height: 0;
        padding: 5px 0 0;
        margin-bottom: 18px;
        float: left;

        .with-x {
          font-size: 12px;
          margin: 0 5px 5px 0;
          display: inline-block;
          overflow: hidden;
          color: #000;
          background: #f7f7f7;
          padding: 0 7px;
          height: 20px;
          line-height: 20px;
          border: 1px solid #dedede;
          white-space: nowrap;
          transition: color 400ms;
          cursor: pointer;

          i {
            margin-left: 10px;
            cursor: pointer;
            font: 400 14px tahoma;
            display: inline-block;
            height: 100%;
            vertical-align: middle;
          }

          &:hover {
            color: #28a3ef;
          }
        }
      }
    }

    .details {
      margin-bottom: 5px;

      .sui-navbar {
        overflow: visible;
        margin-bottom: 0;

        .filter {
          min-height: 40px;
          padding-right: 20px;
          background: #fbfbfb;
          border: 1px solid #e2e2e2;
          padding-left: 0;
          border-radius: 0;
          box-shadow: 0 1px 4px rgba(0, 0, 0, 0.065);

          .sui-nav {
            position: relative;
            left: 0;
            display: block;
            float: left;
            margin: 0 10px 0 0;

            li {
              float: left;
              line-height: 18px;

              a {
                display: block;
                cursor: pointer;
                padding: 11px 15px;
                color: #777;
                text-decoration: none;
              }

              &.active {
                a {
                  background: #e1251b;
                  color: #fff;
                }
              }
            }
          }
        }
      }

      .goods-list {
        margin: 20px 0;

        ul {
          display: flex;
          flex-wrap: wrap;

          li {
            height: 100%;
            width: 20%;
            margin-top: 10px;
            line-height: 28px;

            .list-wrap {
              .p-img {
                padding-left: 15px;
                width: 215px;
                height: 255px;

                a {
                  color: #666;

                  img {
                    max-width: 100%;
                    height: auto;
                    vertical-align: middle;
                  }
                }
              }

              .price {
                padding-left: 15px;
                font-size: 18px;
                color: #c81623;

                strong {
                  font-weight: 700;

                  i {
                    margin-left: -5px;
                  }
                }
              }

              .attr {
                padding-left: 15px;
                width: 85%;
                overflow: hidden;
                margin-bottom: 8px;
                min-height: 38px;
                cursor: pointer;
                line-height: 1.8;
                display: -webkit-box;
                -webkit-box-orient: vertical;
                -webkit-line-clamp: 2;

                a {
                  color: #333;
                  text-decoration: none;
                }
              }

              .commit {
                padding-left: 15px;
                height: 22px;
                font-size: 13px;
                color: #a7a7a7;

                span {
                  font-weight: 700;
                  color: #646fb0;
                }
              }

              .operate {
                padding: 12px 15px;

                .sui-btn {
                  display: inline-block;
                  padding: 2px 14px;
                  box-sizing: border-box;
                  margin-bottom: 0;
                  font-size: 12px;
                  line-height: 18px;
                  text-align: center;
                  vertical-align: middle;
                  cursor: pointer;
                  border-radius: 0;
                  background-color: transparent;
                  margin-right: 15px;
                }

                .btn-bordered {
                  min-width: 85px;
                  background-color: transparent;
                  border: 1px solid #8c8c8c;
                  color: #8c8c8c;

                  &:hover {
                    border: 1px solid #666;
                    color: #fff !important;
                    background-color: #666;
                    text-decoration: none;
                  }
                }

                .btn-danger {
                  border: 1px solid #e1251b;
                  color: #e1251b;

                  &:hover {
                    border: 1px solid #e1251b;
                    background-color: #e1251b;
                    color: white !important;
                    text-decoration: none;
                  }
                }
              }
            }
          }
        }
      }

      .page {
        width: 733px;
        height: 66px;
        overflow: hidden;
        float: right;

        .sui-pagination {
          margin: 18px 0;

          ul {
            margin-left: 0;
            margin-bottom: 0;
            vertical-align: middle;
            width: 490px;
            float: left;

            li {
              line-height: 18px;
              display: inline-block;

              a {
                position: relative;
                float: left;
                line-height: 18px;
                text-decoration: none;
                background-color: #fff;
                border: 1px solid #e0e9ee;
                margin-left: -1px;
                font-size: 14px;
                padding: 9px 18px;
                color: #333;
              }

              &.active {
                a {
                  background-color: #fff;
                  color: #e1251b;
                  border-color: #fff;
                  cursor: default;
                }
              }

              &.prev {
                a {
                  background-color: #fafafa;
                }
              }

              &.disabled {
                a {
                  color: #999;
                  cursor: default;
                }
              }

              &.dotted {
                span {
                  margin-left: -1px;
                  position: relative;
                  float: left;
                  line-height: 18px;
                  text-decoration: none;
                  background-color: #fff;
                  font-size: 14px;
                  border: 0;
                  padding: 9px 18px;
                  color: #333;
                }
              }

              &.next {
                a {
                  background-color: #fafafa;
                }
              }
            }
          }

          div {
            color: #333;
            font-size: 14px;
            float: right;
            width: 241px;
          }
        }
      }
    }
  }
}
</style>